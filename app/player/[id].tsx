...
export default function PlayerScreen() {
  ...
  useEffect(() => {
    if (sectionKey) {
      listVideosByCategory(sectionKey).then((videos) => setSectionVideos(videos));
    }
  }, [sectionKey]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.titleContainer}>
        <Text style={styles.title} numberOfLines={2}>{currentVideo.title}</Text>
        {sectionVideos.length > 1 && (
          <Text style={styles.progress}>{currentIndex + 1} of {sectionVideos.length}</Text>
        )}
      </View>
      {sectionVideos.length > 1 && (
        <View style={styles.controls}>
          ...
          >
            <Text style={{ fontSize: 20, color: currentIndex === 0 ? "#6B7280" : "#fff" }}>⏮</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.navButton, styles.nextButton, currentIndex >= sectionVideos.length - 1 && styles.navButtonDisabled]}
            onPress={handleNext}
            disabled={currentIndex >= sectionVideos.length - 1}
          >
            <Text style={{ fontSize: 20, color: currentIndex >= sectionVideos.length - 1 ? "#6B7280" : "#fff" }}>⏭</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  titleContainer: { position: "absolute", top: 50, left: 70, right: 16, zIndex: 100 },
  title: {
    fontSize: 16, fontWeight: "600", color: "#fff",
    textShadowColor: "rgba(0,0,0,0.8)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3,
  },
  progress: {
    fontSize: 12, color: "#E5E7EB", marginTop: 4,
    textShadowColor: "rgba(0,0,0,0.8)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3,
  },
  ...
  controls: {
    ...
    flexDirection: "row", justifyContent: "center", gap: 40, zIndex: 100,
  },
  navButton: {
    width: 56, height: 56, borderRadius: 28,
    backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "center", alignItems: "center",
  },
  nextButton: { backgroundColor: "#8B5CF6" },
  navButtonDisabled: { opacity: 0.5 },
  doneEmoji: { fontSize: 64, marginBottom: 16 },
  doneTitle: { fontSize: 28, fontWeight: "bold", color: "#fff", marginBottom: 8 },
  doneSubtitle: { fontSize: 16, color: "#9CA3AF", marginBottom: 32 },
  primaryButton: {
    flexDirection: "row", alignItems: "center", backgroundColor: "#8B5CF6",
    paddingHorizontal: 24, paddingVertical: 14, borderRadius: 8, gap: 8, marginBottom: 12,
  },
  secondaryButton: {
    paddingHorizontal: 24, paddingVertical: 14, borderRadius: 8,
    borderWidth: 1, borderColor: "#374151",
  },
  secondaryButtonText: { color: "#9CA3AF", fontWeight: "600", fontSize: 16, textAlign: "center" },
});
